...
export default function PublicProviderOrderForm() {
  ...
  // Submit order mutation (public version)
  const submitOrderMutation = useMutation({
    mutationFn: async (data: { orderData: any; pdfBase64: string }) => {
      const response = await apiRequest("POST", "/api/submit-order-public", data);
      return response.json();
    },
    onSuccess: () => {
      // Redirect to success page
      setLocation("/public-order-success");
    },
    onError: (error: Error) => {
      toast({
  ...
  const generatePDFBlob = async (): Promise<{ doc: jsPDF; blob: string }> => {
    try {
      ...
      doc.text("5290 NW 33rd Ave", 190, 25, { align: "right" });
      doc.text("Fort Lauderdale, FL 33309", 190, 32, { align: "right" });
      doc.text("Phone: (954) 593-0374", 190, 39, { align: "right" });
      
      // Title
      let yPos = 55;
      doc.setFontSize(16);
      doc.setFont("helvetica", "bold");
      doc.text("Provider Order Form", 105, yPos, { align: "center" });
      yPos += 20;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Header with NXT Medical branding */}
      <div className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4">
            <div className="flex items-center">
              <img src={nxtLogo} alt="NXT Medical" className="h-12 w-auto" />
              <div className="ml-4">
                <h1 className="text-2xl font-bold text-gray-900">Provider Order Form</h1>
                <p className="text-sm text-gray-600">Tissue Graft Ordering System</p>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <Input
                  ...
                  placeholder="e.g., 8/8/2025 4PM EST"
                  className="mt-1"
                />
              </div>
            </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            <div className="flex items-center space-x-2">
              ...
              />
              <Label htmlFor="billingDifferent">Billing information is different from shipping</Label>
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      placeholder="Enter billing fax number"
                      className="mt-1"
                    />
                  </div>
                  <div>
              ...
              {orderItems.map((item, index) => (
                <Card key={item.id} className="p-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div>
                      <Label>Select Graft</Label>
                      <Select 
                        value={item.graftName ? `${item.graftName}-${graftData.find(g => g.graftName === item.graftName && g.totalSqCm.toString() === item.totalSqCm)?.sizesSqCm || ''}` : ""} 
                        onValueChange={(value) => handleGraftSelection(item.id, value)}
                      >
                        <SelectTrigger className="mt-1">
                          <SelectValue placeholder="Choose a graft" />
                        </SelectTrigger>
                        <SelectContent>
                          {graftData.map((graft, index) => (
                            <SelectItem key={index} value={`${graft.graftName}-${graft.sizesSqCm}`}>
                              {graft.graftName} ({graft.sizesSqCm}) - {formatCurrency(graft.totalCost.toString())}
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                    ...
                    <div>
                      <Label htmlFor={`productCode-${item.id}`}>Product Code</Label>
                      <Input
                        id={`productCode-${item.id}`}
