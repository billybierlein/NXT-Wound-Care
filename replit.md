...
- **Referral Sources Management**: Main referral sources table displays Facility, Contact, Type, Referrals Sent (sum of kanban referrals + patient referrals), Sales Rep, and Actions columns. Volume and Status columns removed for streamlined view focused on actual referral activity metrics. Sortable Facility (alphabetical A-Z/Z-A) and Referrals Sent (ascending/descending) columns with visual sort indicators (ArrowUp, ArrowDown, ArrowUpDown icons). Default sort: Facility ascending.
