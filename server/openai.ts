...
// Contextual help generation for wound care application
export async function generateContextualHelp(context: {
  page: string;
  section: string;
  field?: string;
  userRole?: string;
  ...
  try {
    const { page, section, field, userRole, currentData } = context;

    // Build context-specific prompts based on page and section
    const contextPrompts: Record<string, string> = {
      'add-patient': 'Focus on proper patient intake procedures, required documentation, and wound care assessment protocols.',
      'patient-profile': 'Emphasize comprehensive patient management, treatment tracking, and clinical documentation best practices.',
      'patient-treatments': 'Highlight treatment planning, outcome tracking, and evidence-based wound care interventions.',
      'manage-providers': 'Cover provider relationship management, credentialing, and care coordination protocols.',
      'referral-sources': 'Address referral management, care transitions, and inter-facility communication standards.',
      'calculator': 'Explain revenue calculations, reimbursement considerations, and practice economics for wound care.',
      'ai-assistant': 'Guide users on leveraging AI tools for clinical decision support and educational content generation.'
    };

    const sectionPrompts: Record<string, string> = {
      'patient-info': 'Patient demographics, medical history, and wound characteristics documentation.',
      'treatment-form': 'Treatment planning, product selection, and intervention documentation.',
      'timeline': 'Patient progress tracking, outcome measurement, and longitudinal care coordination.',
      'wound-assessment': 'Systematic wound evaluation, measurement techniques, and healing indicators.',
      'billing': 'Documentation requirements, coding considerations, and reimbursement optimization.',
      'commission': 'Sales tracking, performance metrics, and compensation calculations.'
    };

    const rolePrompts: Record<string, string> = {
      'admin': 'Provide comprehensive system management guidance with focus on oversight and analytics.',
      'sales_rep': 'Emphasize field activities, provider relationships, and performance tracking.',
      'provider': 'Focus on clinical protocols, documentation standards, and patient care optimization.'
    };

    let systemPrompt = `You are an expert wound care consultant providing contextual help for a medical application. Generate helpful, accurate guidance that is:
...
${contextPrompts[page] || 'Provide general wound care application guidance.'}
${sectionPrompts[section] || ''}
${userRole ? rolePrompts[userRole] || '' : ''}

RESPONSE FORMAT:
Return a JSON object with:
- insights: A concise, actionable explanation (2-3 sentences)
- tips: Array of 2-3 practical tips
- bestPractices: Array of 2-4 key best practice terms
- relatedConcepts: Array of 2-4 related medical/workflow concepts

Focus on practical, actionable guidance that helps users work more effectively.`;

    if (field) {
      systemPrompt += `\n\nSPECIFIC FIELD GUIDANCE: Provide targeted help for the "${field}" field.`;
    }

    if (currentData) {
      systemPrompt += `\n\nCURRENT DATA CONTEXT: ${JSON.stringify(currentData, null, 2)}`;
    }
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        { role: "system", content: systemPrompt },
        { role: "user", content: `Generate contextual help for this specific situation. Provide practical, role-appropriate guidance that helps the user understand and work effectively with this part of the wound care application.` }
      ],
      max_tokens: 800,
      temperature: 0.2, // Low temperature for consistent, reliable help content
      response_format: { type: "json_object" }
    });

    const helpContent = JSON.parse(response.choices[0].message.content || '{}');
    
    // Ensure all required fields are present with fallbacks
    return {
      insights: helpContent.insights || "This feature helps streamline your wound care workflow.",
      tips: Array.isArray(helpContent.tips) ? helpContent.tips : ["Check documentation requirements", "Follow clinical protocols"],
      bestPractices: Array.isArray(helpContent.bestPractices) ? helpContent.bestPractices : ["Evidence-based care", "Proper documentation"],
      relatedConcepts: Array.isArray(helpContent.relatedConcepts) ? helpContent.relatedConcepts : ["Clinical assessment", "Care coordination"]
    };
  } catch (error) {
    console.error("Error generating contextual help:", error);
    
    // Return fallback help content
    return {
      insights: "This feature supports your wound care management workflow with best practices and clinical guidance.",
      tips: ["Follow established protocols", "Document thoroughly", "Consider patient safety"],
      bestPractices: ["Evidence-based care", "Proper documentation", "Care coordination"],
      relatedConcepts: ["Clinical assessment", "Treatment planning", "Outcome tracking"]
    };
  }
}